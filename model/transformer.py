"""
A small, dependency-light GPT-style decoder-only transformer for Parentheses 0.9.

Deliberately close to nanoGPT/LLaMA-style architecture (RMSNorm, RoPE, SwiGLU
MLP) since that combination is well-understood, well-optimized in PyTorch,
and cited throughout docs/training-time-estimate.md for throughput
calibration. Kept in a single file so it's easy to read end to end.
"""
...
class Parentheses(nn.Module):
    ...
    def hidden(self, idx: torch.Tensor, cache: list | None = None) -> torch.Tensor:
...
def self_test():
    ...
    Run with `python -m model.transformer` (relative imports need the package).
    """
    torch.manual_seed(0)
